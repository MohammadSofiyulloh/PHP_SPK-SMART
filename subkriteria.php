...
<form method="post">
    ...
    <div class="input-control text full-size">
        <input type="text" name="nama" placeholder="Nama Sub Kriteria"
            value="<?php echo isset($_GET['nama'])? $_GET['nama'] : ''; ?>">
    </div>
    <label>Nilai</label>
    <div class="input-control text full-size">
        <input type="text" name="nilai" placeholder="Nilai Sub Kriteria"
            value="<?php echo isset($_GET['nilai'])? $_GET['nilai'] : ''; ?>">
    </div>
    <label> Gol Kriteria</label>
    <div class="input-control select full-size">
        <select name="kriteria">
            <option value="<?php echo isset($_GET['kriteria'])? $_GET['kriteria'] : ''; ?>">
                <?php echo isset($_GET['kriteria'])? $_GET['kriteria'] : ''; ?></option>
            <?php
        ...
        <tr>
            ...
            ?>
                ...
                <a href="?page=form&id=<?php echo $row2['id_sub_kriteria'] ?>&nama=<?php echo $row2['nama_sub_kriteria'] ?>&nilai=<?php echo $row2['nilai_sub_kriteria'] ?>&kriteria=<?php echo $row2['id_kriteria'] ?>"
                    style="color:orange;"><span class="mif-pencil icon"></span></a>
                <a href="?page=hapus&id=<?php echo $row2['id_sub_kriteria'] ?>" style="color:red;"><span
                        class="mif-cancel icon"></span></a><br />
                <?php
            }
            ?>
            </td>
